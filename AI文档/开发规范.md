...
*   小程序使用openai登录
*   后端收到 `wxCode` 后，调微信接口静默换取 `openid`，将其 upsert 至 `sysUser.wx_openid`，用户无感知。
*   签发同款 JWT，小程序端存储于 `Taro.setStorageSync('token', ...)`，请求时携带 `Authorization: Bearer <token>`。
*   `sysUser` 表新增字段：`wx_openid VARCHAR(64) NULL UNIQUE COMMENT '微信小程序 openid'`。

### 10.2 API 请求规范

*   封装统一请求工具（`services/request.ts`），基于 `Taro.request`，自动注入 token、处理 Envelope 响应、统一错误提示。
*   复用后端同一套 REST 接口，不为小程序单独新建接口，确需差异化时在现有接口入参中扩展。


### 10.4 分包规范

*   主包（`pages/`）体积不超过 1.5MB，超出必须拆分为分包（`subPackages`）。
*   低频功能（报表、设置、WebView 页）优先下沉分包。
...
*   E2E：登录 + 列表查询 + 新增/编辑/删除至少 1 条关键链路
