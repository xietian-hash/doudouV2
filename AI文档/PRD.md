...
| ------------ | ---------- | ---------- | ------------------------ |
| v0.1.0       | 谢天       |            | 创建文档                 |
| v0.2.0       | 谢天       | 2026-06-20 | 新增重复账单功能         |
| v0.3.0       | 谢天       | 2026-08-12 | 补充记录页选中日期记账规则 |
| v0.4.0       | 谢天       | 2026-08-12 | 支持子分类调整所属一级分类 |
...
| US-09 | 作为用户，我希望设置重复账单，让固定收支自动按日/月/年周期生成，以便减少手工记账负担 | 可配置重复类型（日/月/年）、重复日期、金额、分类、账户、开始日期、结束日期；系统每天凌晨自动生成到期账单；支持启用/暂停和删除重复账单 | P1         | v1.0     |
...
用户可配置按日、按月、按年自动重复的账单模板，系统每天凌晨自动生成到期账单，减少手工记账负担。

### 页面说明

| #   | 对象     | 说明                                                               |
| --- | -------- | ------------------------------------------------------------------ |
| 1   | 页面作用 | 在"我的页"管理所有重复账单，支持新增、编辑、启用/暂停、删除       |
| 2   | 入口     | 我的页"工具"分组中的"重复账单"入口                                 |
| 3   | 列表内容 | 展示分类图标、分类名、金额、重复周期描述、生效区间、启用/暂停状态 |
| 4   | 编辑表单 | 设置收支类型、金额、分类、账户、备注、重复类型、重复日期、生效区间 |

### 业务规则

| #   | 规则             | 说明                                                                                            |
| --- | ---------------- | ----------------------------------------------------------------------------------------------- |
| 1   | 重复类型         | 支持按日（每天）、按月（每月指定几号）、按年（每年指定几月几号）三种类型                        |
| 2   | 重复日期配置     | 按日无需配置日期；按月需指定1-31号；按年需指定月份（1-12）和日期（1-31）                        |
| 3   | 月末特殊处理     | 按月/年配置的日期超过当月实际天数时（如2月31号），自动顺延至当月最后一天生成                    |
| 4   | 生效区间         | 开始日期必填，默认今日；结束日期选填，为空表示永久重复；开始日期不可晚于结束日期                |
| 5   | 自动生成时机     | 每天凌晨00:05服务端定时任务运行，生成当日及此前遗漏的账单                                       |
| 6   | 防漏补生成       | 记录每个重复账单最后一次生成日期，每次运行时从上次生成日期之后开始逐日补算，确保不漏生成        |
| 7   | 防重复生成       | 数据库层对（重复账单ID + 账单日期）建立唯一约束，即使定时任务意外重复执行也不会产生重复账单     |
| 8   | 补生成上限       | 单次运行最多向前追溯90天，防止用户配置很久以前的开始日期时一次性生成大量账单                   |
| 9   | 生成账单来源标记 | 自动生成的账单 `source=3`，并关联 `recurring_bill_id`，用户可在账单详情看到"由重复账单生成"标识 |
| 10  | 启用/暂停        | 暂停期间不生成新账单，但 `last_generated_date` 不更新；恢复启用后从暂停前的日期继续补算          |
| 11  | 删除重复账单     | 软删除重复账单模板，不影响已生成的历史账单                                                      |
| 12  | 编辑重复账单     | 修改重复账单不影响已生成的历史账单，下次生成时以新配置为准                                      |
...
| #   | 对象     | 说明                                                           |
| --- | -------- | -------------------------------------------------------------- |
| 1   | 页面作用 | 用户在"我的页"填写邮箱地址，将全部账单数据导出为 Excel 发送到邮箱 |
| 2   | 入口     | 我的页"数据工具"分组中的"数据导出"入口                           |
| 3   | 邮箱输入 | 必填，需符合邮箱格式                                           |
| 4   | 导出内容 | Excel 附件包含三个工作表：账单明细、分类列表、账户列表         |
| 5   | 发送结果 | 发送成功后 Toast 提示，提示用户注意查收垃圾箱                  |

### 业务规则

| #   | 规则         | 说明                                                                      |
| --- | ------------ | ------------------------------------------------------------------------- |
| 1   | 邮箱校验     | 前后端均校验邮箱格式，格式不正确不可提交                                  |
| 2   | 导出范围     | 导出当前用户默认账本下的全部历史账单，不支持筛选时间范围                  |
| 3   | 附件内容     | Sheet1 账单明细（日期、类型、金额、一级分类、二级分类、账户、备注、标签）；Sheet2 分类列表；Sheet3 账户列表 |
| 4   | 发送方式     | 后端通过 QQ 邮箱 SMTP 发送，邮件标题含导出日期                            |
| 5   | 错误处理     | 邮件发送失败时返回错误提示，用户可重试                                    |
...
| source             | tinyint(1)    | 是   | 来源：1手工录入 2语音识别 3重复账单自动生成 |
| voice_text         | varchar(500)  | 否   | 语音原始文本，语音记账时保存                |
| recurring_bill_id  | bigint        | 否   | 关联重复账单ID，source=3时有值              |
| is_deleted  | tinyint(1)    | 是   | 软删除：0否 1是，默认0       |
| created_at  | datetime      | 是   | 创建时间                     |
| updated_at  | datetime      | 是   | 更新时间                     |

### 重复账单表 (recurring_bills)
...
| amount               | decimal(12,2) | 是   | 金额，必须 > 0                                                    |
| remark               | varchar(255)  | 否   | 备注                                                              |
| repeat_type          | tinyint(1)    | 是   | 重复类型：1按日 2按月 3按年                                       |
| repeat_day           | tinyint       | 否   | 重复日（1-31）；按月和按年时必填                                  |
| repeat_month         | tinyint       | 否   | 重复月（1-12）；按年时必填                                        |
| start_date           | date          | 是   | 生效开始日期                                                      |
| end_date             | date          | 否   | 生效结束日期，为空表示永久重复                                    |
| last_generated_date  | date          | 否   | 最后一次成功生成账单的日期，用于防漏生成和防重复生成的基准日期    |
| is_active            | tinyint(1)    | 是   | 是否启用：1启用 0暂停，默认1                                      |
| is_deleted           | tinyint(1)    | 是   | 软删除：0否 1是，默认0                                            |
| created_at           | datetime      | 是   | 创建时间                                                          |
| updated_at           | datetime      | 是   | 更新时间                                                          |

> `bills` 表与 `recurring_bills` 表通过 `bills.recurring_bill_id` 关联；同时在 `bills` 表上对 `(recurring_bill_id, bill_date)` 建立唯一索引，数据库层保证同一重复账单在同一天只生成一条账单。
...
| 重复账单     | 重复账单列表 | 查看重复账单     | 展示当前用户所有未删除重复账单，显示分类、金额、重复周期描述、生效区间和启用状态                       | P1         | 待开发   | US-09      |
| 重复账单     | 重复账单管理 | 新增重复账单     | 配置重复类型、重复日期、收支类型、金额、分类、账户、备注、开始/结束日期后保存                          | P1         | 待开发   | US-09      |
| 重复账单     | 重复账单管理 | 编辑重复账单     | 修改重复账单所有字段，修改不影响已生成的历史账单                                                       | P1         | 待开发   | US-09      |
| 重复账单     | 重复账单管理 | 启用/暂停        | 切换重复账单的启用/暂停状态；暂停期间不生成新账单                                                      | P1         | 待开发   | US-09      |
| 重复账单     | 重复账单管理 | 删除重复账单     | 软删除重复账单，不影响已生成的历史账单                                                                 | P1         | 待开发   | US-09      |
...
| 9     | 快捷语音记账 | 在记录页长按底部中间按钮录音                               | 语音解析成功后进入确认记账抽屉；确认保存后刷新记录页账单列表、日历和汇总                         |
| 10    | 分页加载   | 进入记录页先加载第一页账单，滚动到底部自动加载下一页       | 切换月份、筛选日期、删除账单或语音保存后重置分页；顶部汇总来自日历汇总，不受当前已加载页数影响   |
...
| 3     | 数据来源 | 分类列表查询当前用户未删除叶子分类；账户默认取默认账户；从记录页选中具体日期进入时默认该日期，其他情况默认今日 |
...
| **#** | **对象** | **交互说明**                   | **逻辑说明**                           |
| ----- | -------- | ------------------------------ | -------------------------------------- |
| 1     | 开始录音 | 在记录页或记账页长按底部中间按钮开始录音 | 页面进入录音中状态，展示录音面板       |
...
| 3     | 二级分类 | 展示二级分类名称和图标             | 可编辑名称、图标和所属一级分类；所属一级分类只能选择同收支类型的一级分类；接口返回已绑定标签                |
| 4     | 新增分类 | 支持新增一级分类或二级分类         | 名称同用户同父级唯一，图标从分类图标库选择；新增支出二级分类后，后端异步调用大模型自动绑定一个经济属性标签 |
| 5     | 编辑分类 | 支持修改分类名称、图标和所属一级分类 | 已有关联账单时仍允许编辑；调整所属一级分类不会修改历史账单及其统计结果                                    |
...
| **#** | **对象**       | **交互说明**                               | **逻辑说明**                                                                   |
| ----- | -------------- | ------------------------------------------ | ------------------------------------------------------------------------------ |
| 1     | 列表内容       | 展示分类图标、分类名、金额、重复周期描述   | 重复周期描述示例："每天"、"每月5日"、"每年3月15日"                             |
| 2     | 生效区间       | 展示开始日期和结束日期                     | 结束日期为空时显示"永久"                                                        |
| 3     | 状态标签       | 显示"启用"或"暂停"状态                     | 启用时标签高亮，暂停时灰色                                                      |
| 4     | 启用/暂停      | 点击状态开关切换                           | 即时生效，无需二次确认；Toast 反馈结果                                          |
| 5     | 编辑入口       | 点击列表项进入编辑页                       | 编辑页带入当前配置                                                              |
| 6     | 删除           | 列表项左滑触发删除二次确认                 | 确认后软删除，不影响已生成的历史账单；Toast 反馈                                |
| 7     | 新增入口       | 页面右上角或底部"新增"按钮进入新增编辑页  | 跳转至重复账单编辑页（新增模式）                                                |

#### 重复账单编辑页（新增/编辑）

| **#** | **对象**     | **交互说明**                                   | **逻辑说明**                                                                         |
| ----- | ------------ | ---------------------------------------------- | ------------------------------------------------------------------------------------ |
| 1     | 收支类型     | 点击"支出/收入"切换                            | 切换后分类列表随类型刷新；已选分类不属于当前类型时清空                               |
| 2     | 金额         | 数字键盘输入                                   | 必须大于0，最多2位小数                                                               |
| 3     | 分类         | 点击选择分类抽屉                               | 只允许选择叶子分类                                                                   |
| 4     | 账户         | 点击选择账户抽屉                               | 默认选中默认账户                                                                     |
| 5     | 备注         | 文本输入，选填                                 | 最多255字                                                                            |
| 6     | 重复类型     | 切换"按日 / 按月 / 按年"                       | 切换时清空相关日期配置                                                               |
| 7     | 重复日       | 按月：选择1-31；按年：选择月份（1-12）和日（1-31） | 按日时不显示此配置；按月/年必填；超出当月天数时提示"不足当月天数时取当月最后一天"   |
| 8     | 开始日期     | 日期选择器，必填，默认今日                     | 不可晚于结束日期                                                                     |
| 9     | 结束日期     | 日期选择器，选填                               | 可清空表示永久；不可早于开始日期                                                     |
| 10    | 保存         | 点击"保存"提交                                 | 前端完成必填校验后提交；保存成功后返回列表页并刷新                                   |
